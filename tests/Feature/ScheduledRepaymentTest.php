...
class ScheduledRepaymentTest extends TestCase
{
    ...
    /**
     * Test the scheduled repayment with success case.
     *
     * @return void
     */
    public function test_scheduled_repayment_successful_response()
    {
        global  $userId, $loanId, $userToken;
        $headers = ['Authorization' => 'Bearer ' . $userToken];
        $payload = ['loan_id' => $loanId, 'user_id' => $userId, 'amount' => 1500,'repayment_method'=>'UPI'];
        $response = $this->json('POST', '/api/payschedulerepayment', $payload, $headers);
        $data = $response->json()['data'];
        $response
            ->assertStatus(200)
            ->assertJson([
    ...
    /**
     * Test the scheduled repayment with failed case.
     *
     * @return void
     */
    public function test_scheduled_repayment_with_wrong_payload_failed_response()
    {
        global  $userId, $loanId, $userToken;
        $headers = ['Authorization' => 'Bearer ' . $userToken];
        $payload = ['loan_id' => '', 'user_id' => $userId, 'amount' => 'amount'];
        $response = $this->json('POST', '/api/payschedulerepayment', $payload, $headers);
        $data = $response->json()['data'];
        $response
            ->assertStatus(200)
